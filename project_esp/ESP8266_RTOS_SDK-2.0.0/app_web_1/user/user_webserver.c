...
webconfig_set_wifi(char* urlparam){
	...
	//ssid=301&password=mwl.17521705230
	...
	memcpy(ssid_tmp, p + 5,sizeof(ssid_tmp));

	for(i ; i < strlen(ssid_tmp) ; i++){
		printf("ssid_tmp[%d]=%d\n",i,ssid_tmp[i]);
		if(ssid_tmp[i] != 38)
			ssid[i] = ssid_tmp[i];
		else
	...
	memcpy(password, q + 9, strlen(urlparam) - (q - urlparam) - 5);
	...
	memcpy(&stConf.password, password, strlen(password));
	...
	*/
    ...
    switch (pURL_Frame->Type) {
        ...
        case POST:
			...
			if((strncmp(pURL_Frame->pSelect, "/wifi_config", 12) == 0)){
				printf("-------wifi_config\n");

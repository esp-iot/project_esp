...
int app_cmd_handler_func(char *cmd){
	...
	else if( strncmp( cmd , "led",3) == 0)
	{
			...
			printf("bytes4 = %02x\n",Data0.bytes4);
			ReadInverter(0x02 , 0x06 ,0x02 , Data0.bytes4);
	}
...
{
	pvTcpServerThreadHandle = sys_thread_new("tcp_server" ,  tcp_server_thread , NULL, 5000 , 9 );//第三个参数设置堆栈，太小会出现：(stack_size =xxx,task handle = xxxxx) overflow the heap_size.的错误

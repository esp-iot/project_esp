...
static void message_arrived_huashengke(MessageData* data)
{
	...
	parse_Json_1(data->message->payload , 0);
    ...
    while (1)
    {
        switch(process_state)
        {
            ...
            case MATT_PUBLISH:
				...
				DEBUG("MATT_PUBLISH message.payload = %s\n",message.payload);
                ...
                break;
            default:
				//Detect the MQTT connection status
				rc = MQTTIsConnected(&client);
				if(rc != 0){//rc != 0 , The connection fails
					//Detect the DUT connection status
					if(wifi_station_get_connect_status( ) != STATION_GOT_IP)//No IP 
						DEBUG("No IP\n");
					process_state = MATT_NETWORK_CONNECT;
				}
